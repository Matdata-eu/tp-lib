...
fn test_csv_to_projection_pipeline() {
    ...
    let network = load_network(network_path).expect("Failed to load network");
    
    // Load test GNSS data (3 positions)
    let gnss_path = "tests/fixtures/test_gnss.csv";
    let gnss_positions = load_gnss(gnss_path, Some("EPSG:4326"), "latitude", "longitude", "timestamp")
    ...
    assert_eq!(projected.len(), gnss_positions.len(), 
    ...
    for pos in &projected {
        assert!(!pos.netelement_id.is_empty(), "netelement_id should not be empty");
        assert!(pos.measure_meters >= 0.0, "measure_meters should be non-negative");
        assert!(pos.projection_distance_meters >= 0.0, "projection_distance should be non-negative");
    ...
    assert!(json_output.contains("FeatureCollection"), "Should be a FeatureCollection");
