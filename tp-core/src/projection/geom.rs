...
mod tests {
    ...
    fn test_calculate_measure_at_start() {
        ...
        assert!(measure < 10.0, "Measure at start should be near 0, got {}", measure);
    }
    ...
    fn test_calculate_measure_at_end() {
        ...
        assert!((measure - expected_length).abs() < 10.0, 
            "Measure at end should be near {}, got {}", expected_length, measure);
    }
    ...
    fn test_calculate_measure_at_middle() {
        ...
        assert!((measure - expected_measure).abs() < 10.0,
            "Measure at middle should be near {}, got {}", expected_measure, measure);
    }
    ...
    fn test_calculate_measure_point_off_line() {
        ...
        assert!((measure - expected_measure).abs() < 1000.0,
            "Measure for point off line should be near {}, got {}", expected_measure, measure);
    }
    ...
    fn test_project_gnss_position() {
        ...
        assert!(projected.projection_distance_meters > 0.0, 
            "Projection distance should be positive, got {}", projected.projection_distance_meters);
        
        // Verify measure is reasonable (somewhere along the linestring, not negative)
        assert!(projected.measure_meters >= 0.0, 
            "Measure should be non-negative, got {}", projected.measure_meters);
        
        // Verify measure is not beyond the end of the linestring
        let start = Point::new(4.0, 50.0);
        let end = Point::new(5.0, 50.0);
        let total_length = start.haversine_distance(&end);
        assert!(projected.measure_meters <= total_length + 1.0,
            "Measure should not exceed linestring length {}, got {}", total_length, projected.measure_meters);
    }
}
