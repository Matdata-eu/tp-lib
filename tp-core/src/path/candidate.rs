...
pub fn find_candidate_netelements(
    ...
    for netelement in netelements {
        ...
        let (distance, intrinsic, proj_point) = calculate_closest_point_on_linestring(
            &gnss_point,
            &netelement.geometry,
        )?;
...
pub fn calculate_heading_at_point(
    ...
    for i in 0..coords.len() - 1 {
        let segment_start = &coords[i];
        let lat_diff = (point.y() - segment_start.y()) * 111320.0;
        let lon_diff = (point.x() - segment_start.x()) * 111320.0 * segment_start.y().to_radians().cos();
        let distance = (lat_diff * lat_diff + lon_diff * lon_diff).sqrt();
...
mod tests {
    ...
    fn test_candidate_selection_within_cutoff() {
        ...
        let gnss = GnssPosition::new(50.8502, 4.3502, chrono::Utc::now().into(), "EPSG:4326".to_string()).unwrap();
    ...
    fn test_candidate_selection_beyond_cutoff() {
        ...
        let gnss = GnssPosition::new(50.90, 4.40, chrono::Utc::now().into(), "EPSG:4326".to_string()).unwrap();
