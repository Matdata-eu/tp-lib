...
pub fn select_best_path(
    ...
    let mut best_idx = 0;
    let mut best_prob = if reaches_end[0] { candidate_probabilities[0] } else { 0.0 };
...
mod tests {
    ...
    fn test_path_probability_single_segment() -> Result<(), ProjectionError> {
        let seg = AssociatedNetElement::new(
            "elem1".to_string(),
            0.85,
            0.0,
            1.0,
        ...
        let prob = calculate_path_probability(&vec![seg]);
    ...
    fn test_path_probability_multiple_segments() -> Result<(), ProjectionError> {
        ...
        let prob = calculate_path_probability(&vec![seg1, seg2]);
    ...
    fn test_path_probability_empty() {
        let prob = calculate_path_probability(&vec![]);
        assert_eq!(prob, 0.0);
    }
